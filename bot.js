const { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, ChannelType, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, AttachmentBuilder, SlashCommandBuilder, REST, Routes } = require('discord.js');
const fs = require('fs');
...
// Config - Railway uses environment variables
const config = {
...
// Anti-scam message tracker
const scamMessageCooldown = new Map();
...
function startAntiScamMessages() {
    setInterval(async () => {
        try {
            ...
            await channel.send({ embeds: [scamEmbed] });
            console.log('✅ Anti-scam message sent to tickets channel');
        } catch (error) {
...
async function createTicket(interaction, type, description) {
    try {
            ...
            try {
                ...
                if (channel) {
                    const errorEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`You already have an open ticket: ${channel}\n\nPlease close it before creating a new one.`)
                        .setColor(0xe74c3c)
        ...
        const loadingEmbed = new EmbedBuilder()
            ...
            .setDescription('Setting up your dedicated support channel')
            .setColor(0x3498db)
            .setTimestamp();
        ...
        // Create professional ticket embed
        const ticketEmbed = new EmbedBuilder()
            ...
            .setColor(0x3498db)
            .setThumbnail('https://media.discordapp.net/attachments/1429234159674593352/1429235801782489160/romels_stock_banner1.png')
            .setFooter({ text: 'Romel\'s Stock • Secure Support', iconURL: guild.iconURL() || client.user.displayAvatarURL() })
            .setTimestamp();
        ...
        await ticketChannel.send({ 
            content: `${member} <@&${config.adminRole}> 🔔 **Staff Notification**`, 
            embeds: [ticketEmbed], 
...
async function sendVouchRequest(user, ticketDescription, staffMember) {
    try {
        const vouchEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { 
                    ...
                    value: 'Select your rating below - your feedback is valuable to us!' 
                }
            )
            .setColor(0x9B59B6)
            .setThumbnail('https://media.discordapp.net/attachments/1429234159674593352/1429235801782489160/romels_stock_banner1.png')
        ...
        setTimeout(() => {
            if (vouchSessions.has(user.id)) {
                vouchSessions.delete(user.id);
            }
        }, 24 * 60 * 60 * 1000);
...
async function sendVouchToChannel(user, rating, ticketDescription, comment = '') {
    try {
        ...
        const ratingText = rating === 5 ? 'Outstanding!' :
                          rating === 4 ? 'Great Service' :
                          rating === 3 ? 'Good Service' :
                          rating === 2 ? 'Fair Experience' : 'Needs Improvement';

        const vouchEmbed = new EmbedBuilder()
            .setTitle(`📝 ${rating >= 4 ? 'Positive' : rating === 3 ? 'Neutral' : 'Constructive'} Review`)
            .setDescription(`### ${stars} **${rating}/5 - ${ratingText}**\n\n**Service:** ${ticketDescription}`)
            .addFields(
        ...
        const message = await vouchChannel.send({ embeds: [vouchEmbed] });
        
        // Add reactions based on rating
        if (rating >= 4) {
            await message.react('✅');
            await message.react('⭐');
        } else if (rating === 3) {
...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        // Handle other interactions (dropdowns, buttons, modals)
        if (!interaction.isStringSelectMenu() && !interaction.isButton() && !interaction.isModalSubmit()) return;

        // Existing interaction handling code continues below...
        // [Rest of your existing interaction handling code remains the same]
